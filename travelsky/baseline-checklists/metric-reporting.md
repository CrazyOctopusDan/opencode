...
- `packages/opencode/script/build-node.ts`：如果继续保留 Desktop sidecar node server 的 `OPENCODE_VERSION` 注入，该值不能再作为当前统计接口 `toolVersion` 或 `ideVersion` 的来源。
...
- `packages/opencode/test/session/metric.test.ts`：回归验证生成记录请求体、生成行数、语言、请求/响应内容、采纳量空内容、provider 过滤与 `toolName` 优先级。
...
- `toolVersion` 必须使用 `packages/opencode/src/session/metric.ts` 中的 `metricToolVersion` 短版本常量；每次合并后必须与本次合入 dev 的 `packages/opencode/package.json` version 一致，不能回退到 `InstallationVersion`。
...
- 采纳量接口必须是 `POST /ai/data/api/record/addAdoption`，body 必须包含 `qaid`、`adoptedLines`、`adoptedContent`、`deletedLines`。
- agent 时代必须以本轮最终文件 diff 作为采纳口径：有新增或删除行即发送采纳量；没有最终文件变更则不发送采纳量。
- 采纳量 `adoptedLines` 必须等于最终 diff 新增行数，`deletedLines` 必须等于最终 diff 删除行数，且类型为 Integer。
...
- 上游如果调整 Desktop sidecar、WSL sidecar、CLI build 或 build-node 构建流程，必须重新确认 `OPENCODE_TOOL_NAME` 仍在生产包链路中可用，同时不能把统计接口 `toolVersion` 或 `ideVersion` 改回构建注入版本。
...
- 在 `packages/opencode` 目录运行：`bun test test/script/build-node.test.ts test/session/metric.test.ts`，验证 CLI build 注入工具名、metric 优先使用显式工具名，并确认统计版本字段不依赖构建注入版本。
...
- 人工检查生成记录请求体，确认 URL 为 `/ai/data/api/record/saveGeneration`，字段为当前平铺 body，`toolVersion` 与本次合入 dev 的 `packages/opencode/package.json` version 一致，`ideVersion` 为空字符串，且不再发送第一版 `other.v1/v2`。
