...
export const DialogManageModels: Component = () => {
    ...
    queryClient.fetchQuery(
      loadProvidersQuery(sdk().scope, sdk().directory, sdk().client, {
        recoverProviderAuth: async () => {
          if (await auth.recover()) return sdk().createClient({ directory: sdk().directory, throwOnError: true })
  ...
  const models = createMemo(() => {
    ...
    const enterprise = [...list.all.values()].filter((item) => item.id === "travelSky" || item.name === "travelSky")
