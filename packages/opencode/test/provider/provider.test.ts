...
it.effect("refreshes TravelSky model policy when provider state was initialized before login", () =>
  Effect.gen(function* () {
    ...
    const inInstance = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(provideInstanceEffect(dir), Effect.provide(InstanceLayer.layer), Effect.provide(CrossSpawnSpawner.defaultLayer))

    const missing = yield* inInstance(
      Provider.use.getProvider(ProviderV2.ID.make("travelSky")).pipe(
        Effect.match({
          onFailure: () => undefined,
    ...
    const model = yield* inInstance(Provider.use.getModel(ProviderV2.ID.make("travelSky"), ModelV2.ID.make("Qwen3.5-27B")))
