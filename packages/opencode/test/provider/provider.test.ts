...
afterEach(async () => {
  ...
  Flag.OPENCODE_TEMPO_BASE_URL = undefined
...
it.instance("refreshes TravelSky model policy when provider state was initialized before login", () =>
  Effect.gen(function* () {
    ...
    const missing = yield* Provider.use.getProvider(ProviderV2.ID.make("travelSky")).pipe(
      Effect.match({
        onFailure: () => undefined,
    ...
    const model = yield* Provider.use.getModel(ProviderV2.ID.make("travelSky"), ModelV2.ID.make("Qwen3.5-27B"))
