...
describe("tui thread", () => {
  test("loads the TUI integration lazily", async () => {
    ...
    const ensureLogin = source.indexOf("const auth = await ensureLogin")
    const requestHeaders = source.indexOf("const requestHeaders = external ? transport.headers : authHeaders(auth)")
    const validateSession = source.indexOf("await validateSession")
    const validateHeaders = source.indexOf("headers: requestHeaders", validateSession)
    const run = source.indexOf("run({")
    const runHeaders = source.indexOf("headers: requestHeaders", run)
    expect(ensureLogin).toBeGreaterThan(-1)
    expect(requestHeaders).toBeGreaterThan(ensureLogin)
    expect(validateSession).toBeGreaterThan(requestHeaders)
    expect(validateHeaders).toBeGreaterThan(validateSession)
    expect(run).toBeGreaterThan(validateHeaders)
    expect(runHeaders).toBeGreaterThan(run)
