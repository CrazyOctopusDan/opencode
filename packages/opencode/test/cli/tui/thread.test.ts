...
describe("tui thread", () => {
  test("loads the TUI integration lazily", async () => {
    ...
    expect(source.indexOf("await ensureLogin")).toBeGreaterThan(-1)
    expect(source.indexOf("await validateSession")).toBeGreaterThan(source.indexOf("await ensureLogin"))
    expect(source).toContain("headers: authHeaders(auth)")
