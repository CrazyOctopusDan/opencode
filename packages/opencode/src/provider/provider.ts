...
export const ListResult = Schema.Struct({
  ...
  debug_tempo: optional(TempoDebug),
})
...
function fromModelPolicyModel(
  provider: ModelPolicyList[number],
  model: ModelPolicyList[number]["models"][number],
): Model {
  return {
    id: ModelV2.ID.make(model.id),
    providerID: ProviderV2.ID.make(provider.id),
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        const raw = yield* Effect.promise(() => ModelPolicy.snapshot())
        const policy = (() => {
          if (!raw.enabled || raw.list.length === 0) return raw
          const all = new Map<string, (typeof raw.list)[number]["models"][number]>()
          const first = raw.list[0]
          for (const item of raw.list) {
            for (const model of item.models) {
              if (all.has(model.id)) continue
              all.set(model.id, {
                ...model,
                baseURL: model.baseURL ?? item.baseURL,
                apiKey: model.apiKey ?? item.apiKey,
              })
            }
          }
          const list = [
          ...
          return {
            ...raw,
            list,
            provider: (id: string) => list.find((item) => item.id === id),
            allowedProvider: (id: string) => list.some((item) => item.id === id),
            allowedModel: (id: string, model: string) =>
              list.some((item) => item.id === id && item.models.some((value) => value.id === model)),
          }
        })()
        const catalog = {
          ...mapValues(modelsDev, fromModelsDevProvider),
          ...(policy.enabled ? fromModelPolicy(policy.list) : {}),
        }
        ...
        function mergeProvider(providerID: ProviderV2.ID, provider: Partial<Info>) {
          ...
          if (existing) {
            providers[providerID] = { ...existing, ...mergeDeep(existing, provider) }
            return
          }
          const match = database[providerID]
          if (!match) return
          providers[providerID] = { ...match, ...mergeDeep(match, provider) }
        }
        ...
        for (const [id, provider] of Object.entries(providers)) {
          ...
          const policies = item ? new Map(item.models.map((value) => [value.id, value])) : undefined

          if (item) {
            const sample = Object.values(provider.models)[0]
            for (const value of item.models) {
              const id = ModelV2.ID.make(value.id)
              if (provider.models[id]) continue
              provider.models[id] = sample
                ? {
                    ...sample,
                    id,
                    providerID,
                ...
                : {
                    ...
                    cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
                    limit: { context: value.contextLength ?? 32000, output: value.maxTokens ?? 8192 },
                    status: "active",
                    options: {},
                    headers: {},
                    release_date: "",
          ...
          for (const [modelID, model] of Object.entries(provider.models)) {
            ...
            const rule = policies?.get(modelID)
            model.api.id = model.api.id ?? model.id ?? modelID
            if (item) {
              model.api.npm = "@ai-sdk/openai-compatible"
              model.api.url = rule?.baseURL ?? item.baseURL
              if (rule?.contextLength) model.limit.context = rule.contextLength
              if (rule?.maxTokens) model.limit.output = rule.maxTokens
              if (rule?.apiKey && !provider.key) provider.key = rule.apiKey
            }
            if (
    ...
    const refreshTravelSkyModel = Effect.fn("Provider.refreshTravelSkyModel")(function* (
      ...
      const travelSkyID = ProviderV2.ID.make("travelSky")
      if (providerID !== travelSkyID) return
      const policy = yield* Effect.promise(() => ModelPolicy.snapshot(true))
      if (!policy.enabled || policy.expired) return
      const provider = fromModelPolicy(policy.list)[travelSkyID]
      if (!provider?.models[modelID]) return
      s.catalog[travelSkyID] = provider
