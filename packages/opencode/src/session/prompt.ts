    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<SessionV1.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        yield* Effect.gen(function* () {
          if (out.info.role !== "assistant" || out.info.finish !== "stop" || out.info.error) return

          const provider = out.info.providerID
          const rows = yield* MessageV2.filterCompactedEffect(sessionID).pipe(
            Effect.provideService(Database.Service, database),
          )
          const latest = MessageV2.latest(rows)
          if (latest.assistant?.id !== out.info.id || latest.user?.id !== out.info.parentID) return
