...
export const GlobalLoginInput = Schema.Struct({
  username: Schema.String,
  password: Schema.String,
})

const GlobalLoginResult = Schema.Struct({
  access_token: Schema.String,
  token_type: Schema.Literal("Bearer"),
  expires_in: Schema.Number,
})

const GlobalLogoutResult = Schema.Struct({
  ok: Schema.Boolean,
})
...
export const GlobalApi = HttpApi.make("global").add(
  HttpApiGroup.make("global")
    .add(
      HttpApiEndpoint.post("login", GlobalPaths.login, {
        payload: GlobalLoginInput,
        success: described(GlobalLoginResult, "Login success"),
        error: HttpApiError.BadRequest,
      }).annotateMerge(
        OpenApi.annotations({
          identifier: "global.login",
          summary: "Login",
          description: "Authenticate with username and password, then get bearer token.",
        }),
      ),
      HttpApiEndpoint.post("logout", GlobalPaths.logout, {
        success: described(GlobalLogoutResult, "Logout success"),
      }).annotateMerge(
        OpenApi.annotations({
          identifier: "global.logout",
          summary: "Logout",
